...

{
  ...
  my $width = $X->width_in_pixels;
  my $height = $X->height_in_pixels;
  $width = 200;
  $height = 200;
  ...
  if ($width == $X->width_in_pixels && $height == $X->height_in_pixels) {
  ...
  my ($pow,$exp) = round_up_pow(max($width,$height)*3, 3);

  my $width_excess = $pow - $width;
  my $height_excess = $pow - $height;
  my $centred_x = int($width_excess/2);
  my $centred_y = int($height_excess/2);

  my $zero = $X->new_rsrc;
  $X->CreatePixmap ($zero, $window, 1, $pow,$pow);
  my $one = $X->new_rsrc;
  $X->CreatePixmap ($one, $window, 1, $pow,$pow);

  my $new_one = $X->new_rsrc;
  $X->CreatePixmap ($new_one, $window, 1, $pow,$pow);
  my $new_zero = $X->new_rsrc;
  $X->CreatePixmap ($new_zero, $window, 1, $pow,$pow);

  my $bit_gc = $X->new_rsrc;
  $X->CreateGC ($bit_gc, $one,
  ...
  my $copy = sub {
    ...
    # return if $x >= $width;
    # return if $y >= $height;
    # my $w = min ($len, $width-$x);
    # my $h = min ($len, $height-$y);
    $X->CopyArea ($from, $to, $bit_gc,
                  0,0,      # src x,y
                  $len,$len,    # width,height
                  $x,$y);   # dst x,y
  };

  $X->ChangeGC($bit_gc, foreground => 0);
  $X->PolyPoint ($one, $bit_gc, 'Origin', 0,0);
  $X->ChangeGC($bit_gc, foreground => 1);
  $X->PolyPoint ($zero, $bit_gc, 'Origin', 0,0);

  foreach (1 .. $exp) {
    $copy->($one, $new_zero, 0,0);
    $copy->($one, $new_zero, 0,1);
    $copy->($one, $new_zero, 0,2);
    $copy->($one, $new_zero, 1,0);
    $copy->($one, $new_zero, 1,1);
    $copy->($one, $new_zero, 1,2);
    $copy->($one, $new_zero, 2,0);
    $copy->($one, $new_zero, 2,1);
    $copy->($one, $new_zero, 2,2);

    $copy->($zero, $new_one, 0,0);
    $copy->($one, $new_one, 0,1);
    $copy->($zero, $new_one, 0,2);
    $copy->($one, $new_one, 1,0);
    $copy->($zero, $new_one, 1,1);
    $copy->($one, $new_one, 1,2);
    $copy->($zero, $new_one, 2,0);
    $copy->($one, $new_one, 2,1);
    $copy->($zero, $new_one, 2,2);

    $len *= 3;
    ($one,$new_one) = ($new_one,$one);
    ($zero,$new_zero) = ($new_zero,$zero);
  }
  print "final len $len\n";

  print "screen ${width}x${height} round up to $pow\n";
  print "excess $width_excess,$height_excess centred x,y $centred_x,$centred_y\n";

  my $draw_x = $centred_x;
  my $draw_y = $centred_y;
  # -500,-500,
  # $width_excess,$height_excess,

  my @keysym_arefs =  $X->GetKeyboardMapping
    ($X->{'min_keycode'},
     $X->{'max_keycode'} - $X->{'min_keycode'} + 1);
  print "keycode table ",scalar(@keysym_arefs),"\n";
  ...
  $X->{'event_handler'} = sub {
    ...
    if ($h{'name'} eq 'KeyPress') {
      my $keycode = $h{'detail'};
      ### $keycode
      my $keysym = $keysym_arefs[$keycode - $X->{'min_keycode'}]->[0];
      ### keysym_aref: $keysym_arefs[$keycode - $X->{'min_keycode'}]
      ### keysym: sprintf '%X', $keysym
  ...
  for (;;) {
    ...
    ### $redraw
    if ($redraw) {
      $X->CopyPlane ($one, $window, $gc,
                     $draw_x,$draw_y,
                     $width,$height,    # whole window
                     0,0,   # dst x,y
                     1);  # bit plane
    }
  }
  ...
  sub round_up_pow {
    my ($n, $radix) = @_;
    my ($pow, $exp) = round_down_pow($n-1, $radix);
    return ($pow*$radix, $exp+1);
  }
}
